...
### Bravo pour ce script très bien documenté!
### Il fonctionne super bien, sinon pour trois erreurs, que je t'explique ci-dessous
...
for chiffre in range(1000,2001):
	...
	if req.status_code != 200:         # cela done bien [200] donc tout fonctionne jusque là ! OUF parce que maintenant ça se corse !!! 
		...
		if infos["type"]== "audio": # merci pour le tuyau ici :) !!!!
			    ...
			    listes.append(infos["titre"].split(" /")) ### Le dernier caractère de cette ligne est une parenthèse (qui ferme les arguments de la méthode .append) et non un crochet
			    ### Après le split ci-dessus, il fallait consigner, dans la liste «infos», uniquement le premier élément résultant du split.
			    ...
			    ### Il manquait l'info sur l'URL. Voir corrigé pour trouver le bon chemin.

		### Il faut enfin indenter ces trois dernières lignes d'un coup à droite
		### afin qu'elles se trouvent à l'intérieur de ta condition «infos["type"]== "audio":»
		### sinon, ton script va ajouter une ligne au CSV, même quand le document n'est pas de type audio
		### en fait, il va ajouter quelques centaines de fois les infos relatives au dernier fichier audio rencontré
		f2 = open(fichier,"a")
		prout= csv.writer(f2)
